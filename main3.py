...
def load_pu_fashionmnist(root, pos_class_indices, labeled_positive_num, batch_size):
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))
    ])

    dataset_train = MyFashionMNIST(root=root, train=True, download=True, transform=transform)
    true_binary_labels = [1 if t in pos_class_indices else 0 for t in dataset_train.targets]
    dataset_train.true_binary_labels = true_binary_labels
    prior = sum(true_binary_labels) / len(true_binary_labels)
    ...
    dataset_test = MyFashionMNIST(root=root, train=False, download=True, transform=transform)
    test_true_binary = [1 if t in pos_class_indices else 0 for t in dataset_test.targets]
    dataset_test.true_binary_labels = test_true_binary
    dataset_test.targets = test_true_binary
...
if __name__ == '__main__':
    ...
    # 数据集相关
    DATA_ROOT = 'data'
    POS_CLASS_INDICES = [0, 2, 4, 6]  # 定义正类 (飞机, 汽车, 船, 卡车)
    ...
    # --- 2. 设置 (数据, 模型, 优化器, 调度器) ---
    train_loader, test_loader, class_prior = load_pu_fashionmnist(
        root=DATA_ROOT,
