...
class MyPCAM(PCAM):
    def __init__(self, root, split='train', transform=None, download=False):
        ...
        self.true_binary_labels = None  # 为 PU 标签准备的字段

    def __getitem__(self, idx):
        img, pu_label = super().__getitem__(idx)
        true_label = self.true_binary_labels[idx] if self.true_binary_labels else -1
        return img, pu_label, true_label, idx

def load_pu_pcam(root: str, labeled_positive_num: int, batch_size: int):
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))  # PCAM是RGB图像也可以设为3通道均值/方差
    ])

    # 加载数据集
    dataset_train = MyPCAM(root=root, split='train', transform=transform, download=True)
    true_binary_labels = dataset_train.targets.numpy().tolist()
    dataset_train.true_binary_labels = true_binary_labels
    prior = sum(true_binary_labels) / len(true_binary_labels)
    print(f"PCAM 数据集正类比例估计 (pi_p): {prior:.4f}")
    ...
    # 构造测试集（用真实标签评估）
    dataset_test = MyPCAM(root=root, split='test', transform=transform, download=True)
    test_true_labels = dataset_test.targets.numpy().tolist()
    dataset_test.true_binary_labels = test_true_labels
    dataset_test.targets = test_true_labels
...
class PULoss(nn.Module):
    ...
    def __init__(self, prior: float, gamma: float = 1.0, loss_fn: nn.Module = nn.BCEWithLogitsLoss(reduction='none'),
        ...
        if adaptive_base_weight < 0.0 or adaptive_base_weight >= 1.0: raise ValueError("Adaptive base weight must be between 0.0 and 1.0 (exclusive of 1.0).")
        ...
        print(f"PULoss 初始化为 *{loss_type}*, 先验={self.prior:.4f}, gamma={self.gamma:.2f}, epsilon={self.adaptive_epsilon:.1e}")
    ...
    def forward(self, logits: torch.Tensor, pu_labels: torch.Tensor) -> torch.Tensor:
        ...
        if num_u == 0: return self.prior * risk_p_plus if num_p > 0 else torch.tensor(0.0, device=device, requires_grad=True)
        ...
        else: risk_u_component = risk_u_as_neg_elements.mean()
...
def evaluate_model(model: nn.Module, test_loader: DataLoader, device: torch.device) -> Tuple[float, float, float]:
    ...
    if len(np.unique(all_true_labels)) > 1:
        try: auc = roc_auc_score(all_true_labels, all_preds_probs)
        except ValueError as e: print(f"计算 AUC 时出错: {e}")
    else: print("警告: 测试集中仅存在单一类别标签, AUC 无法计算。")
    precision, recall, f1, _ = precision_recall_fscore_support(all_true_labels, predicted_labels, average='binary', pos_label=1, zero_division=0)
    print(f"测试集评估结果:")
...
if __name__ == '__main__':
    ...
    if USE_ADAPTIVE_IN_STAGE2:
        print(f"阶段二自适应损失参数: gamma={GAMMA}, epsilon={ADAPTIVE_EPSILON}, base_weight={ADAPTIVE_BASE_WEIGHT_STAGE2}")
    print(f"使用早停: 是, 耐心值: {EARLY_STOPPING_PATIENCE}")
    print(f"----------------")


    # --- 2. 设置 (数据, 模型, 优化器, 调度器) ---
    train_loader, test_loader, class_prior = load_pu_pcam(
        root=DATA_ROOT,
