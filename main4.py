...
def get_resnet18_for_binary_classification() -> nn.Module:
    """ 获取一个用于二分类的ResNet18模型。"""
...
class MyEuroSAT(torchvision.datasets.ImageFolder):
    def __init__(self, root, transform=None):
        super().__init__(root=root, transform=transform)
        self.true_binary_labels = None

    def __getitem__(self, index):
        img, pu_label = super().__getitem__(index)
        true_label = self.true_binary_labels[index] if self.true_binary_labels else -1
        return img, pu_label, true_label, index  # ✅ 返回4个元素


def load_pu_eurosat(root: str, pos_class_indices: List[int], labeled_positive_num: int, batch_size: int):
    transform = transforms.Compose([transforms.ToTensor()])
    dataset = MyEuroSAT(root=root, transform=transform)

    # 标记正类
    true_binary_labels = [1 if label in pos_class_indices else 0 for label in dataset.targets]
    dataset.true_binary_labels = true_binary_labels
    prior = sum(true_binary_labels) / len(true_binary_labels)

    print(f"EuroSAT 数据集加载成功，总样本数: {len(dataset)}，正类比例: {prior:.4f}")

    # 按 80-20 划分训练集和测试集
    train_size = int(0.8 * len(dataset))
    test_size = len(dataset) - train_size
    _train_dataset, _test_dataset = torch.utils.data.random_split(dataset, [train_size, test_size])

    train_dataset = copy.deepcopy(dataset)
    train_dataset.targets = np.array(train_dataset.targets)[_train_dataset.indices].tolist()
    train_dataset.samples = np.array(train_dataset.samples)[_train_dataset.indices].tolist()
    train_dataset.imgs = np.array(train_dataset.imgs)[_train_dataset.indices].tolist()
    train_dataset.true_binary_labels = np.array(train_dataset.true_binary_labels)[_train_dataset.indices].tolist()

    test_dataset = copy.deepcopy(dataset)
    test_dataset.targets = np.array(test_dataset.targets)[_test_dataset.indices].tolist()
    test_dataset.samples = np.array(test_dataset.samples)[_test_dataset.indices].tolist()
    test_dataset.imgs = np.array(test_dataset.imgs)[_test_dataset.indices].tolist()
    test_dataset.true_binary_labels = np.array(test_dataset.true_binary_labels)[_test_dataset.indices].tolist()

    # 标记训练集中部分正例
    pos_indices = [i for i, val in enumerate(train_dataset.true_binary_labels) if val == 1]
    labeled_pos_indices = random.sample(pos_indices, min(labeled_positive_num, len(pos_indices)))
    pu_labels = [1 if i in labeled_pos_indices else 0 for i in range(len(train_dataset))]
    train_dataset.targets = pu_labels

    print(f"训练集样本数: {len(train_dataset)}，标记正样本数: {sum(pu_labels)}，未标记样本数: {len(train_dataset) - sum(pu_labels)}")
...
if __name__ == '__main__':
    ...
    POS_CLASS_INDICES = [1]  # 定义正类 (飞机, 汽车, 船, 卡车)
    ...
    # --- 2. 设置 (数据, 模型, 优化器, 调度器) ---
    train_loader, test_loader, class_prior = load_pu_eurosat(
        root=DATA_ROOT,
